...
$$
\text{serverOffset} = T_{\text{ntp}} - T_{\text{local\_mid}} = \delta_S
$$

RTT が対称であれば、$T_{\text{local\_mid}}$ は NTP 応答瞬間のサーバー時計の読みと一致するため、差分はそのままサーバーのずれ $\delta_S$ になる。

**Phase 2** で求まる clientOffset:

$$
\text{ntpTimeAtServer} = \text{serverTime} + \delta_S = (\text{真の時刻} - \delta_S) + \delta_S = \text{真の時刻}
$$

$$
\text{clientMid} = \text{真の時刻} - \delta_C \quad (\text{RTT対称の場合})
$$

$$
\text{clientOffset} = \text{真の時刻} - (\text{真の時刻} - \delta_C) = \delta_C
$$

**Phase 3** で得られる ntpNow():

$$
\text{ntpNow()} = \text{Date.now()} + \delta_C = (\text{真の時刻} - \delta_C) + \delta_C = \text{真の時刻}
$$
...
$$
\text{誤差} \leq \frac{|d_{\text{行き}} - d_{\text{帰り}}|_{\text{UDP}}}{2} + \frac{|d_{\text{行き}} - d_{\text{帰り}}|_{\text{HTTP}}}{2}
$$
